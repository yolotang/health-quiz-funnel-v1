...
- **填写过程中的辅助卡片（Step 2 子步）**  
  - **当前体重**：在身高、体重合法时，展示基于 BMI 的简要分类说明（`BmiInsightCard`），仅作科普级参考，非医疗诊断。  
  - **目标体重**：在校验通过时，根据与 WHO 常用区间一致的「健康体重带」提示是否偏低（`GoalWeightLowAlertCard`），或与当前体重差异较大时的「变化幅度」提示（`GoalWeightGetMovingCard`）；与减脂/增肌/塑形目标冲突时以表单校验为准，不叠加营销卡片。  
  - **身高**：含健康数据用途的同意勾选（`healthOnboardingConsent`），体重步依赖同一同意状态方可继续。
